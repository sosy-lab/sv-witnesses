...
| sink | *Valid values:* ``false`` (default) or ``true`` <br />  This state is a sink. All paths that lead to this state end here and should not be further explored by the witness validator. Nodes where this flag is set must not have any leaving edges | Yes | No |
