...
class WitnessLint:
    ...
    def __init__(
        self, witness, program, check_program, check_callstack, ignore_self_loops
    ):
    ...
    def log_with_position(self, level, lineno, msg, *args):
        if level >= LOGLEVELS["warning"]:
            self.exit_code = 1
        logging.getLogger("with_position").log(
            level, msg, *args, extra={"line": lineno}
        )
    ...
    def check_functionname(self, name, pos):
        ...
        if self.program_info is not None:
            if name not in self.program_info["function_names"]:
                self.log_with_position(
                    LOGLEVELS["warning"],
                    pos,
                    "'%s' is not a functionname of the program",
                    name,
                )
        else:
    ...
    def check_linenumber(self, line, pos):
        ...
        if self.program_info is not None:
            if int(line) < 1 or int(line) > self.program_info["num_lines"]:
                self.log_with_position(
                    LOGLEVELS["warning"], pos, "%s is not a valid linenumber", line
                )
        else:
        ...
        if self.program_info is not None:
            if int(offset) < 0 or int(offset) >= self.program_info["num_chars"]:
                self.log_with_position(
                    LOGLEVELS["warning"],
                    pos,
                    "%s is not a valid character offset",
                    offset,
                )
        else:
        ...
        if key == "entry":
            ...
            if data.text == "false":
                ...
                self.log_with_position(
                    LOGLEVELS["warning"],
                    data.sourceline,
                    "Invalid value for key 'violation': %s",
                    data.text,
                )
            self.violation_witness_only.add(key)
        elif key == "invariant":
            ...
            self.log_with_position(
                LOGLEVELS["warning"],
                data.sourceline,
                "Unknown key for node data element: %s",
                key,
            )
        ...
        if key == "assumption":
            ...
            if data.text not in ["condition-true", "condition-false"]:
                self.log_with_position(
                    LOGLEVELS["warning"],
                    data.sourceline,
                    "Invalid value for key 'control': %s",
                    data.text,
                )
            self.violation_witness_only.add(key)
        elif key == "startline":
            ...
            if data.text == "false":
                ...
                self.log_with_position(
                    LOGLEVELS["warning"],
                    data.sourceline,
                    "Invalid value for key 'enterLoopHead': %s",
                    data.text,
                )
            ...
            if data.text not in self.threads:
                self.log_with_position(
                    LOGLEVELS["warning"],
                    data.sourceline,
                    "Thread with id %s doesn't exist",
                    data.text,
                )
            ...
            if data.text in self.threads:
                self.log_with_position(
                    LOGLEVELS["warning"],
                    data.sourceline,
                    "Thread with id %s has already been created",
                    data.text,
                )
            else:
            ...
            self.log_with_position(
                LOGLEVELS["warning"],
                data.sourceline,
                "Unknown key for edge data element: %s",
                key,
            )
        ...
        if key == "witness-type":
            if data.text in ["correctness_witness", "violation_witness"]:
                ...
                self.log_with_position(
                    LOGLEVELS["warning"],
                    data.sourceline,
                    "Invalid value for key 'witness-type': %s",
                    data.text,
                )
            ...
            if data.text in ["C", "Java"]:
                ...
                self.log_with_position(
                    LOGLEVELS["warning"],
                    data.sourceline,
                    "Invalid value for key 'sourcecodelang': %s",
                    data.text,
                )
        elif key == "producer":
            ...
            self.log_with_position(
                LOGLEVELS["warning"],
                data.sourceline,
                "Unknown key for graph data element: %s",
                key,
            )
    ...
    def handle_key(self, key):
        ...
        if "id" in key.attrib:
            ...
            self.log_with_position(
                LOGLEVELS["warning"], key.sourceline, "Key is missing attribute 'id'"
            )
            key_id = None
        if "for" in key.attrib:
            ...
            self.log_with_position(
                LOGLEVELS["warning"], key.sourceline, "Key is missing attribute 'for'"
            )
            key_domain = None
        if key_id and key_domain:
            if key_id in self.defined_keys:
                self.log_with_position(
                    LOGLEVELS["warning"],
                    key.sourceline,
                    "Found multiple key definitions with id '%s'",
                    key_id,
                )
            else:
        ...
        if len(key) > 1:
            self.log_with_position(
                LOGLEVELS["warning"],
                key.sourceline,
                "Expected key to have at most one child but has %s",
                len(key),
            )
        for child in key:
                ...
                if key_id in ["entry", "sink", "violation", "enterLoopHead"]:
                    if not child.text == "false":
                        self.log_with_position(
                            LOGLEVELS["warning"],
                            key.sourceline,
                            "Default value for %s should be 'false'",
                            key_id,
                        )
            else:
                self.log_with_position(
                    LOGLEVELS["warning"],
                    child.sourceline,
                    "Invalid child for key element: %s",
                    child.tag,
                )
    ...
    def final_checks(self):
        ...
        for key in self.used_keys.difference(set(self.defined_keys)):
            if key in COMMON_KEYS:
                ...
                self.log(
                    LOGLEVELS["warning"], "Key '%s' has been used but not defined", key
                )
        for key in set(self.defined_keys).difference(self.used_keys):
            self.log(
                LOGLEVELS["info"],
                "Unnecessary definition of key '%s', key has never been used",
                key,
            )
        if self.witness_type is None:
        ...
        if self.witness_type == "correctness_witness":
            for key in self.violation_witness_only:
                self.log(
                    LOGLEVELS["warning"],
                    "Key '%s' is not allowed in correctness witness",
                    key,
                )
            ...
            for key in self.correctness_witness_only:
                self.log(
                    LOGLEVELS["warning"],
                    "Key '%s' is not allowed in violation witness",
                    key,
                )
    ...
    def lint(self):
        ...
        try:
            ...
            self.log_with_position(
                LOGLEVELS["critical"], err.lineno, "Malformed witness:\n\t%s", err.msg
            )
